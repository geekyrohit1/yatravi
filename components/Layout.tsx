...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Handle body scroll locking when mobile menu, search, or other popups are open
  useEffect(() => {
    const isAnyPopupOpen = isMobileMenuOpen || showSearchModal || isQuotePopupOpen || isExternalPopupOpen || isExternalFormOpen;
  ...
  }, [isMobileMenuOpen, showSearchModal, isQuotePopupOpen, isExternalPopupOpen, isExternalFormOpen]);

  // CRITICAL: Always reset scroll/interactivity on route change
  useEffect(() => {
    setIsMobileMenuOpen(false);
    setShowSearchModal(false);
    document.body.style.overflow = '';
    
    // Small delay to ensure the new content is ready before allowing scroll
    const timer = setTimeout(() => {
      window.ytvLenis?.start();
    }, 100);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white opacity-100">
      ...
      {/* AI Assistant Chat Widget (Temporarily disabled by user request) */}
      {mounted && !loading && false && settings?.enableAIChat === true && !isHideIconsPage && (
        <ViChatAssistant isHidden={isQuotePopupOpen || isExternalPopupOpen || isExternalFormOpen} />
      )}

      {/* Floating WhatsApp Button (Force enabled with Smart Visibility) */}
      {mounted && !loading && !isHideIconsPage && !isQuotePopupOpen && !isExternalPopupOpen && !isExternalFormOpen && (
        <div className={`fixed right-6 z-50 flex flex-col items-center gap-2 transition-all duration-700 ${
          showWhatsApp ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12 pointer-events-none'
        } ${hasStickyMobileBar ? 'bottom-[115px]' : 'bottom-10 md:bottom-12'}`}>
          
          <a
            href={`https://wa.me/${(settings?.whatsappNumber || '9587505726').replace(/[^0-9]/g, '')}`}
            target="_blank"
            rel="noopener noreferrer"
            className={`bg-[#25D366] text-white p-3 md:p-3 rounded-full shadow-[0_8px_30px_rgba(37,211,102,0.4)] hover:shadow-[0_12px_40px_rgba(37,211,102,0.3)] transition-all duration-500 flex items-center justify-center hover:scale-110 active:scale-95 group`}
            aria-label="Chat on WhatsApp"
          >
            <svg className="w-8 h-8 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
